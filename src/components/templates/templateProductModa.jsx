...
const TemplateProductModa = ({ data }) => {

    const [cantidad, setCantidad] = useState(1);

    const [talla, setTalla] = useState('');

    function handleTallaSelection(item) {
        setTalla(item);
    }
    ...
    return (
        <div className="border p-4 rounded-lg h-max">
            ...
            <div className="my-2 flex flex-col justify-start">
                <h2 className="text-base font-normal mb-3">Selecciona tu talla:</h2>
                <div className="flex items-center justify-evenly">
                    {data.metadata.talla.split(",").map(
                        (item, i) => (
                            <label key={i} className={`w-10 h-10 rounded-lg border border-gray-300 flex items-center justify-center cursor-pointer mr-2 ${talla === item ? 'bg-gray-200' : ''}`}>
                                <input type="radio" name="talla" value={item} className="hidden" onChange={() => handleTallaSelection(item)} checked={talla === item} />
            ...
            <div className="my-3">{talla && <p className="text-gray-500 mt-2">Talla seleccionada: {talla.toUpperCase()}</p>}</div>
            <CheckOunt total={data.price * cantidad} price={data.price} products={[{ price: data.id_price, quantity: cantidad }]} metadataProduct={{ product: data.name, talla: talla }} />
            <div>
...
export default TemplateProductModa;